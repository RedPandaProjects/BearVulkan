...
VKFrameBuffer::VKFrameBuffer(const BearFrameBufferDescription& description):Description(description)
{
	...
	for (; CountRenderTarget < 8; CountRenderTarget++)
	{
		...
		BEAR_ASSERT(textures->GetType() == TT_RenderTarget);
	...
	BEAR_ASSERT(RenderPassRef->CountRenderTarget == CountRenderTarget);
...
VkRenderPassBeginInfo VKFrameBuffer::GetRenderPass()
{
	...
	for (bsize i = 0; i < CountRenderTarget; i++)
	{
...
void VKFrameBuffer::ToTexture(VkCommandBuffer Cmd)
{
...
void VKFrameBuffer::ToRT(VkCommandBuffer Cmd)
{
	for (bsize i = 0; CountRenderTarget > i; i++)
	{
		auto texture = static_cast<VKTexture2D*>(Description.RenderTargets[i].get());
		TransitionImageLayout(Cmd, texture->Image, texture->ImageInfo.format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, texture->ImageInfo.mipLevels, texture->ImageInfo.extent.depth);
	}
}
